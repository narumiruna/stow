...
- [x] Run one accumulated final interactive batch only after all implementation and non-interactive checks pass: execute `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer Scripts/ui_tests.sh`, capture the complete macOS Library and Settings screenshot matrix from the passing result bundle, regenerate `docs/release/assets/stow-macos-library.png` and approved new Settings assets, inspect every screenshot for clipping/overlap/ambiguous truncation/disruptive shifts, and compare existing Quick Panel screenshots and workflows for regressions; if the batch fails, group related fixes and rerun the complete batch rather than individual UI tests.
  - Evidence: the final batch passed 11/11 macOS and 13/13 iOS UI tests; all Library/Settings attachments were extracted and inspected, approved assets were regenerated, and existing Quick Panel scenarios remained green.

- [x] Audit the final diff and plan evidence: compare every changed file against this scope, confirm no Quick Panel presentation/interaction source changed, confirm no SwiftData migration or preference loss was introduced, confirm the pre-existing untracked `package-lock.json` was not incorporated, append concise command/screenshot evidence to each completed checkbox, and request renewed approval before continuing if any major scope, architecture, risk, or acceptance criterion changed.
  - Evidence: final `git diff --check` passes; the scoped diff contains no Quick Panel presentation or schema changes; UserDefaults keys remain stable; `package-lock.json` is absent; no major plan change was required.

## Completion Checklist

- [x] The macOS Library has Inbox, Recently Used, Pinned, Archive, and Trash management with no duplicate Settings destination. Evidence: focused policy tests and the final macOS navigation scenario pass.
- [x] At 840×560, 1080×720, and a wide Library window, app-owned labels, filters, statuses, actions, footer copy, and decision-relevant values are not clipped, obscured, or ambiguously truncated. Evidence: frame assertions pass and approved short/long screenshots were inspected.
- [x] Library filtering exposes type/source/date criteria, active summaries, removal, clear, loading/empty/error distinctions, and usable narrow behavior. Evidence: focused state tests and final screenshots pass review.
- [x] Library browsing does not mutate retrieval history; explicit use actions do; editing has distinct Cancel/Save Changes, dirty confirmation, atomic success, and draft-preserving failure. Evidence: counter/editing tests and final editing scenarios pass.
- [x] Single and multi-item Pin, Archive/Restore, Move to Trash, Undo, and Trash Restore behavior is deterministic, correctly labeled, and never styles Restore as destructive. Evidence: lifecycle policy tests and final Trash/Restore scenarios pass.
- [x] Native macOS Settings is organized into Capture, Paste & Shortcuts, Sync & Storage, and Privacy; all relevant loading, available, denied, paused, conflict, success, and failure states wrap and remain actionable at minimum width. Evidence: approved light/dark/accessibility screenshots and containment assertions pass.
- [x] Shortcut replacement is atomic and preserves both prior working registrations and stored values on conflict. Evidence: fake-registrar transaction tests and the final shortcut-conflict UI scenario pass.
- [x] Search-index recovery in Settings matches support documentation and preserves the previous usable state on failure. Evidence: deterministic rollback tests pass and the final recovery screenshot shows actionable retained state.
- [x] Existing item/attachment data, unknown persisted fields, UserDefaults keys, local-only/CloudKit behavior, Trash retention, and iOS/iPadOS compatibility are preserved without a schema migration. Evidence: 54 core tests, 24 app tests, schema inspection, and unsigned macOS/iOS builds pass.
- [x] The current Quick Panel visual and interaction design is unchanged. Evidence: no Quick Panel presentation source changed; all existing Quick Panel UI scenarios pass and screenshots match the approved design.
- [x] `Scripts/ci.sh` and the one final `Scripts/ui_tests.sh` batch pass on the required toolchain. Evidence: final runs exit successfully with 54 core, 24 app, 11 macOS UI, and 13 iOS UI tests passing.
- [x] README, support, accessibility audit, test matrix, App Store screenshot metadata, and the new Library/Settings visual review match the implemented experience. Evidence: source-to-document and asset-reference inspection pass.
- [x] The final diff contains no unrelated changes and does not include the pre-existing untracked `package-lock.json`. Evidence: final status/diff review and `git diff --check` pass, and the lock file is absent.
- [x] Every Plan and Completion Checklist item contains current verification evidence; the plan status is `DONE` and this file is archived without overwriting an existing file. Evidence: final checklist audit completed 2026-08-11.
