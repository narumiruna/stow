...
- Symptom: `Bump Version` fails before project steps while resolving Python/uv actions or caches. Cause: external Python version tooling adds irrelevant manifests and action references to this Swift repository. Fix: keep the version in `VERSION` and use the repository-owned `Scripts/bump_version.sh` without Python or uv.
