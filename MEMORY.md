...
- Symptom: the Quick Panel closes when Stow resigns active without a pointer interaction. Cause: `applicationDidResignActive` treats every resignation as an outside click. Fix: use resignation only as a pointer-down fallback; global/local pointer monitors own actual outside clicks.
- Symptom: hosted macOS UI tests close the Quick Panel after evaluating an `XCUIScreenshot`. Cause: hosted screenshot capture perturbs the transient panel, while release captures are produced in the final local UI batch. Fix: lazily skip screenshot expression evaluation in hosted CI with `STOW_CAPTURE_UI_TEST_SCREENSHOTS=0`; keep all interaction assertions enabled.
