...
The workflow uses `Scripts/bump_version.sh` to update `VERSION` and every Xcode `MARKETING_VERSION`, then atomically pushes one `chore(release)` commit and an annotated `vMAJOR.MINOR.PATCH` tag. The project generator reads `VERSION` directly; version bumps do not rewrite its source.
